...
//    onSerialRead(data);
      ...
      if (data != null && data.length > 0) {
        ...
        for (int i = 0; i < data.length-1; i++) {
          datai[i] = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, i);
          countBytes++;
        }

        onSerialRead(data, datai);

        String out = "";
        for (int i = 0; i < data.length-1; i++) {
          out += " " + i + "|"  + characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, i);
//          System.out.println("~~~~~~~~> " + characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, i));
        }
        System.out.print("~~~~~~~~~~> INPUT DATA" + out);
  ...
  private void onSerialRead(byte[] data, int[] datai) {
    if (listener != null)
      listener.onSerialRead(data, datai);
  }
